...
function listerMessages($idConv)
{
	...
	$SQL= "Select messages.contenu,users.pseudo, users.couleur from messages where idConversation='$idConv' INNER JOIN users ON idAuteur=users.id WHERE messages.idConversations='$idConv' AND users.blacklist=0 ORDER BY messages.id ASC";
	return parcoursRS(SQLSelect($SQL));
}
